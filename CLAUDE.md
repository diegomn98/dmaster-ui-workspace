...
- **Auditoría de peso (`npm run size`, `scripts/size-audit.mjs`)**: mide el coste tree-shakeado real por componente contra el FESM del dist. **Gotcha crítico**: el FESM de ng-packagr es compilación parcial (`ɵɵngDeclare*`) y NO es tree-shakeable tal cual — medirlo directo con esbuild da ~74 kB gzip para CUALQUIER import individual (falso positivo de "no tree-shakea"). Hay que pasar primero el linker de Angular + los plugins babel del CLI (`adjust-static-class-members`, `elide-angular-metadata`, `pure-toplevel-functions`, cargados por ruta absoluta desde `@angular/build` porque su `exports` no los expone). Con el pipeline correcto: button ~3.8 kB gzip, tabla completa en el README de la lib (sección "Bundle size" — actualizarla si un componente engorda). La cifra "librería completa" (~94 kB gzip a 45 componentes) y las por-componente del README salen de este script. **El mapa `ENTRIES` del script es manual**: cada componente nuevo se añade ahí o la auditoría lo omite en silencio (pasó con los 10 de las tandas 0.6→0.7).
